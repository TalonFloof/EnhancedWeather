...
public class EnhancedWeather implements ModInitializer {
	...
	public static float getBaseHeat(ServerWorld world, int x, int z) {
		...
		if(temperature < 0.6F) {
			return MathHelper.lerp(temperature/0.6F,0F,50F);
		} else {
			return MathHelper.lerp((temperature-0.6F)/0.4F,50F,100F);
		}
	}
	...
	public static float getHeat(ServerWorld world, int x, int z) {
		...
		return base * time * (heatNoise.GetNoise((float) (noiseTick / 200),0)+1f);
	}
	...
	public static float getHumidity(ServerWorld world, int x, int z) {
		return (getBaseHumidity(world,x,z) * 0.7F + 40 * 0.3F) * (humidityNoise.GetNoise((float) (noiseTick / 200),0)+1f);
	}
